...
!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('indprd', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'indprd',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here

        call write_var(unit_input,unit_output,'class',class)
        call write_var(unit_input,unit_output,'prod',prod(yaml%col_print,yaml%lev_print,:))
        call write_var(unit_input,unit_output,'nprod',nprod)
        call write_var(unit_input,unit_output,'y',y(yaml%col_print,yaml%lev_print,:))
