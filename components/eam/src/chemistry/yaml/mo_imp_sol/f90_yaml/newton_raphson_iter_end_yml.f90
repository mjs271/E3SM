#ifdef YAML_NEWTON_RAPHSON
  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        call write_var(unit_output,'lsol',lsol)
        call write_var(unit_output,'solution',solution)
        call write_var(unit_output,'converged',converged)
        call write_var(unit_output,'convergence',convergence)
        call write_var(unit_output,'prod',prod)
        call write_var(unit_output,'loss',loss)
        call write_var(unit_output,'max_delta',max_delta)
...
! find if any run does not converge. All converged in the testing run
!if(masterproc .and. (.not. convergence) ) then
